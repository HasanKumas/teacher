...
$(document).ready(function(){
    teacherTable = $("#tablesTeacher").DataTable({
        ajax: {
            url: 'api/teachers',
            dataSrc:''
        },
...
function displayTeachers(teachers) {
     $('#teacherHeadContainer').empty();
     $('#teacherBodyContainer').empty();

     $('#teacherHeadContainer').html("<b>Teachers Table</b>");
     $('#teacherHeadContainer').append('<tr><th>Id</th><th>Name</th><th>Actions</th></tr>');
     $.each(teachers, function(index, teacher) {
        $('#teacherBodyContainer').append('<tr><td>' + teacher.id + '</td><td>' +  teacher.name +
            '</td><td><button class = "remove-button" teacherId = " ' + teacher.id + ' " >Delete </button></td></tr>');
     });
     $("#teacherBodyContainer .remove-button").click(removeTeacher);
}

function displayCourses(courses) {
     $('#courseHeadContainer').empty();
     $('#courseBodyContainer').empty();

     $('#courseHeadContainer').html('<b>Courses Table</b>');
     $('#courseHeadContainer').append('<tr><th>Id</th><th>Name</th><th>Teacher Name</th><th>Actions</th></tr>');
     $.each(courses, function(index, course) {
         $('#courseBodyContainer').append('<tr><td>' + course.id + '</td>' + '<td>' +
         course.name + '</td><td>' + course.teacher.name + '</td><td><button class = "remove-button" courseId = " ' + course.id + ' " >Delete </button></td></tr>');
     });
     $("#courseBodyContainer .remove-button").click(removeCourse);
}
